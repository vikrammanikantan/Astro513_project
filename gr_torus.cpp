//========================================================================================
// Athena++ astrophysical GRMHD code
// Copyright(C) 2022 Adapted by Vikram Manikantan <vik@arizona.edu> for powerlaw disks
// Copyright(C) 2014 James M. Stone <jmstone@princeton.edu> and other code contributors
// Licensed under the 3-clause BSD License, see LICENSE file for details
//========================================================================================
//! \file gr_torus.cpp
//! \brief Problem generator for magnetized Chakrabarti torus.
...
// Declarations - figure out which functions need to be changed
...
Real l_mom(Real lam);
Real F_func(Real lam, Real r, Real sth2);
Real LogH(Real r, Real sth);
...
Real r1, r2;                                  // more torus parameters (optional)
Real xi;                                      // rotational scale factor
...
bool powerLaw;                                // Whether to implement a power law solution
                                              //   instead of the FM_torus
Real I;                                       // function of xi, needed later       
} // namespace

//----------------------------------------------------------------------------------------
// Function for preparing Mesh
// Inputs:
//   pin: input parameters (unused) (from athinput file with initial conditions)
// Outputs: (none)
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  if (std::strcmp(COORDINATE_SYSTEM, "kerr-schild") != 0) {
    ...
    msg << "### FATAL ERROR in Problem Generator\n"
        << "GR power law torus only supports Kerr-Schild coordinates" << std::endl;
    ATHENA_ERROR(msg);
    return;
  }
  ...
  powerLaw = pin->GetBoolean("problem", "powerLaw");
  r1 = pin->GetReal("problem", "r1");
  r2 = pin->GetReal("problem", "r2");
  xi = 0.0708;


  I = sqrt((1.0-SQR(xi)/r2)/(1.0-SQR(xi)/r1));
  if (r_peak < 0.0)  {
  ...
  if (r2 < 0 || r1 < 0){
    r1 = 42.0; // the default settings for our disk
    r2 = 1000.0;
  }
...
// Outputs: (none)
// Notes:
//   Initializes power law torus from Chakrabarti 1985.
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  // Initialize primitive values by looping through ever coordinate and calculating every quantity
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      ...
      for (int i = il; i <= iu; ++i) {
        ...
        // TODO: THIS IS WHERE THE INITIAL CONDITIONS ARE SET
        // Calculate background primitives

        // rho = rho_0 * r^(rho_power)
...
void MeshBlock::UserWorkBeforeOutput(ParameterInput *pin) {
  ...
  // Go through all cells 
  // TODO: may have to edit some definitions of u, b and gamma.
  for (int k = ks; k <= ke; ++k) {
    for (int j = js; j <= je; ++j) {
      pcoord->CellMetric(k, j, is, ie, g, gi);
      for (int i = is; i <= ie; ++i) {
...
void TransformContravariantFromBoyerLindquist(Real at_bl, Real ar_bl, Real ath_bl,
    ...
    Real *p_a3) {
  if (std::strcmp(COORDINATE_SYSTEM, "kerr-schild") == 0) {
    Real delta = SQR(x1) - 2.0 * m * x1 + SQR(a); //just add SQR(Q) for kerr-newman metric
    *p_a0 = at_bl + 2.0 * m * x1 / delta * ar_bl;
    *p_a1 = ar_bl;
    *p_a2 = ath_bl;
    *p_a3 = aph_bl + a / delta * ar_bl;
  } 
  else if(std::strcmp(COORDINATE_SYSTEM, "reissner-nordstrom") == 0) {
    double Q = 0; //temporary until Q is implemented
    Real delta = SQR(x1) - 2.0 * m * x1 + SQR(Q);
    *p_a0 = at_bl + 2.0 * m * x1 / delta * ar_bl;
    *p_a1 = ar_bl;
    *p_a2 = ath_bl;
    *p_a3 = aph_bl + a / delta * ar_bl;
  }
  else if(std::strcmp(COORDINATE_SYSTEM, "kerr-newman") == 0) {
    double Q = 0; //temporary until Q is implemented
    Real delta = SQR(x1) - 2.0 * m * x1 + SQR(a) + SQR(Q);
    *p_a0 = at_bl + 2.0 * m * x1 / delta * ar_bl;
    *p_a1 = ar_bl;
    *p_a2 = ath_bl;
    *p_a3 = aph_bl + a / delta * ar_bl;
...
// TODO: new calculation for angular momentum profile needed here
...
// TODO: new calculation for pressure profile needed here
namespace {
Real CalculateRPeakFromL(Real l_target) {
  // Parameters
...
//----------------------------------------------------------------------------------------
// Namespace that contains the new implementation of a large, 
//  power-law angular momentum disk that follows the basic
//  prescription of Penna et al. 2014 and Chakrabarti 1985.
// Developed by Vikram Manikantan (vik@arizona.edu)
namespace{
//----------------------------------------------------------------------------------------
// Function for helping to calculate the angular momentum profile in Penna et al. 2013
// Inputs: 
  Real l_mom(Real lam)
  {
    // discontinuous momentum profile
    if (lam < r1) { lam = r1; }
    else if (lam > r2) { lam = r2; }

    Real G = 1.0 - 2.0/lam;

    return xi*sqrt(lam)/G;
  }



//----------------------------------------------------------------------------------------
// Function for helping to calculate the variable F in Penna et al. 2013
// Inputs: lam - cylindrical radius of the coordinate
//         r - the radial coordinate
//         sth2 - the square of the sin(theta)

  Real F_func(Real lam, Real r, Real sth2){
    // we use the analytical solution for F, which is pretty good
    
    Real l = l_mom(lam);
    Real g_tt = -1 + 2 * m / r;
    Real g_pp = SQR(r) * sth2;
    Real omega = -l * g_tt / g_pp;
    
    return 1 - omega * l;
    }

//----------------------------------------------------------------------------------------
// Function for helping to calculate enthalpy for a power-law disk (chakrabarti; penna)
// Inputs:
...
//   References Penna et al. 2013, ApJ 207 962 (FM).
//   Implements equations (4)-(16) of Penna et al. 2013.

  Real LogH(Real r, Real sth){
    // assuming non-spinning black hole
    ...
    Real lam = sqrt(-g_pp / g_tt);
    Real omega = -l * g_tt / g_pp;

    Real F_ = F_func(lam, r, sth2);
    Real A_ = sqrt(-g_tt - g_pp * SQR(omega));
